...
class OperatorConsoleRuntimeBridgeTests(unittest.TestCase):
    ...
    def test_claim_rewards_workflow_can_queue_run_and_apply_editor_state(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(queued_pane.focus_step_summary, "等待：開啟每日獎勵")
            self.assertEqual(queued_pane.focus_anchor_summary, "daily_ui.claim_reward")
            self.assertEqual(queued_pane.focus_region_summary, "10,20,240,120")
            self.assertEqual(queued_pane.focus_threshold_summary, "0.96")
            ...
            self.assertEqual(pane.focus_step_summary, "全部完成")
            self.assertEqual(pane.focus_anchor_summary, "daily_ui.claim_reward")
            self.assertEqual(pane.focus_region_summary, "10,20,240,120")
            self.assertEqual(pane.focus_threshold_summary, "0.96")
    ...
    def test_claim_rewards_workflow_reports_failed_step_and_failure_surface(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(pane.focus_step_summary, "確認獎勵狀態")
            self.assertIn("daily_ui.reward_confirm_state", pane.focus_anchor_summary)
            self.assertNotEqual(pane.focus_region_summary, "-")
            self.assertRegex(pane.focus_threshold_summary, r"^\d+\.\d{2}$")
            ...
            self.assertIn(pane.focus_region_summary, pane.selected_anchor_summary)
            self.assertIn(pane.focus_threshold_summary, pane.selected_anchor_summary)
    ...
    def test_claim_rewards_pane_disables_actions_when_runtime_readiness_is_blocked(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(pane.focus_step_summary, "執行條件未就緒")
            self.assertEqual(pane.focus_anchor_summary, "daily_ui.claim_reward")
