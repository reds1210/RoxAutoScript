...
- final worktree branch: `codex/feature-guild-order-submit-clean`
- rebuilt from the latest integration base and kept to guild-order-submit changes only
...
- keep follow-up work on `codex/feature-guild-order-submit-clean` and request Codex automatic review or `@codex review` on the PR
